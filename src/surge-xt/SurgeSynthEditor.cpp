...
#include "gui/panels/DeepSynthPanel.h"
    ...
    : juce::AudioProcessorEditor(&p), processor(p)
{
    ...
    // Initialize DeepSynth Sidebar
    deepSynthPanel = std::make_unique<Surge::GUI::Panels::DeepSynthPanel>(
        sge.get(), &(processor.surge->storage));
    addChildComponent(*deepSynthPanel);
    deepSynthPanel->setVisible(false);
}
...
void SurgeSynthEditor::resized()
{
    ...
    auto sidebarWidth = 300;

    if (showDeepSynthSidebar && deepSynthPanel)
    {
        auto sidebarBounds = b.removeFromRight(sidebarWidth);
        deepSynthPanel->setBounds(sidebarBounds);
    }

    topLevelContainer->setBounds(b);
    drawExtendedControls = sge->getShowVirtualKeyboard();
...
void SurgeSynthEditor::toggleDeepSynthSidebar()
{
    showDeepSynthSidebar = !showDeepSynthSidebar;

    if (deepSynthPanel)
    {
        deepSynthPanel->setVisible(showDeepSynthSidebar);
    }

    if (showDeepSynthSidebar)
    {
