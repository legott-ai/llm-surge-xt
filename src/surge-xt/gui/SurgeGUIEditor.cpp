...
void SurgeGUIEditor::toggleDeepSynthSidebar()
{
    if (juceEditor)
    {
        juceEditor->toggleDeepSynthSidebar();
    }
}
...
void SurgeGUIEditor::openOrRecreateEditor()
{
    ...
    {
        deepSynthPanel =
            std::make_unique<Surge::GUI::Panels::DeepSynthPanel>(this, &synth->storage);
        ...
        int deepSynthX =
            currentSkin->getWindowSizeX() - deepSynthWidth - 10; // Right edge with margin
        int deepSynthY =
            currentSkin->getWindowSizeY() - deepSynthHeight - 10; // Bottom edge with margin
